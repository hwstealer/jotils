...
if __file__.endswith("jotils.py"):
    ...
    from randName import randName  # pyright: ignore[reportUnusedImport]  # noqa: F401
    ...
    try: from .extras import *  # noqa: I001
    except ImportError: pass # Running without extras.


# --------------------------- HEX STUFF ---------------------------

def hexs2s(hexstr: str, sep: str ="") -> str:
...
def hexi2s(hexint: int, sep: str ="") -> str:
    "int to hex string (ex. 262254561 -> 'FA1AFE1')"
    if not sep: return f"{hexint:0X}"
    _hex = ["", "0"][((hexint.bit_length()+3)//4 or 1)%2] + f"{hexint:0X}"
    return sep.join([c1+c2 for c1,c2 in zip(_hex[::2],_hex[1::2])])

def hexb2s(hexbytes: bytes, sep: str ="") -> str:
...
def splitBy(obj: list[Any], size: int) -> list[list[Any]]:
...
def swap(cur: Any, obj1: Any, obj2: Any):
...
exit = type("", (), {"__repr__": exit, "__call__": exit})()


def get1st[T](i: Iterable[T]) -> T:
...
def func_rename(func: Callable[[Any], Any], name: str):
...
def randStrId(charCount: int) -> str:
    ...
    return firstChar + "".join([
        string.printable[random.randint(0, 61)] for _ in range(charCount-1)
        ])


def dbgPayload(totSize: int):
...
h2h: Callable[[Any], Any] = lambda i: i
n2n = h2h
...
def notepad(s: Any, pause: bool = True):
    ...
    if pause:
        ...
        with tempfile.TemporaryFile("wb", delete_on_close=False) as fp:
            fp.write(s)
            fp.close()
            subprocess.run(["notepad", fp.name], check=False)
        return

    def _tmp():
        nonlocal s
        # Guard if non-blocking so we don't spawn a million
        if subprocess.run(["tasklist", "/FI", "IMAGENAME eq notepad.exe", "/FO", "CSV", "/NH"], stdout=subprocess.PIPE, check=False).stdout.count(b"\n") > 13: return
        notepad(s)

    Thread(target=_tmp).start()


def funcMap(prefix: str, scopeDict: dict[str, Any], cast: Callable[[Any], Any] | None = None) -> dict[str, Callable[..., Any]]:
...
def consumeBuf(buf: StringIO) -> Any:
...
def rid(obj: str | bytes | list[Any] | tuple[Any] | set[Any], x: Any, *replaceWith: Any):
    "Rid (or replace) 'obj' of instances 'x', preserving type(obj)"
    if isinstance(obj, (str or bytes)):
        if replaceWith:
            return obj.replace(x, replaceWith[0])
        return obj.replace(x, obj[:0])
...
def dictify(obj: Any) -> dict[Any, Any]:
...
def _qview(obj: Any, depth: int = 1, height: int = 20, width: int = 80, _indent: int =0, _offset: int =0):
    height-= 1
    ...
    print(trunc(f"{type(obj)}: {obj!r}", width-_offset))
    ...
    for key in obj:  # pyright: ignore[reportUnknownVariableType]
        if (
            (hasattr(obj[key], "__iter__") and depth > 1) and  # pyright: ignore[reportUnknownArgumentType]
            ...
            any(x for x in obj[key] if (not isPrimitive(x)) or hasattr(x, "__iter__"))  # pyright: ignore[reportUnknownArgumentType, reportUnknownVariableType]
        ...
        print(trunc(f"{' '*_indent}{key!r}: {obj[key]!r}", width))
        height-= 1
...
def qview(obj: Any, depth: int = 1, height: int = 20, width: int = 78):
    "Provide a quick view of the object"
    if height == 1:
        print(trunc(f"{type(obj)}: {obj!r}", width))
...
def catSep(name: str, prefix: str="", lineLen: int=67):
...
catSepCode: Callable[[str], None] = lambda name: catSep(name, "# ")
catSepTxt: Callable[[str], None] = lambda name: catSep(name)


def x2iter(x: Any):
    "Return an object where next(x2iter(x)) is x"
    return iter((x,))


# Print statement with built-in arg-caching
_printData: dict[str, tuple[Any]] = {}
def print(*args: Any, **kwargs: Any):
    key = " ".join([(arg if isinstance(arg, str) else repr(arg)) for arg in args])
    _printData[key] = args
    builtins.print(key, **kwargs)

def getPrint(key: str | None = None) -> Any:
        ...
        while True:
            ...
            for obj in ts:
                if isinstance(obj, templatelib.Interpolation):
                    block+= str(next(objDict[obj.value]))  # pyright: ignore[reportUnknownArgumentType]
                    continue
                block+= obj
            retStr+= block+"\n"
...
def localJotils():
    ...
    if __file__.endswith("jotils.py"):
        ...
        spec = importlib.util.spec_from_file_location("jotils", globals().get("jotilsFile"))
        if not spec or not spec.loader: raise RuntimeError("Failed to load main jotils library")
